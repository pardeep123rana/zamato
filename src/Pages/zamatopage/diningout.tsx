...
const Diningout: React.FC = () => {
  ...
  const fetchDrinks = () => {
    fetch(`https://www.thecocktaildb.com/api/json/v1/1/search.php?s=Blue`)
      .then((response) => response.json())
      .then((data) => {
  ...
  return (
    <div className="flex flex-col h-screen w-screen bg-white">
      <div className="bg-white shadow ">
        <nav className="flex flex-col md:flex-row items-center justify-around bg-white p-4 md:p-10 w-full h-16 md:h-20 ">
          ...
          <div className="flex flex-col md:flex-row md:items-center space-y-4 md:space-y-0 md:space-x-6">
            ...
            >
              kd from agra{" "}
              <span className="text-gray-600 md:inline-block hidden">|</span>
            </h1>
            <input
              type="text"
              placeholder="Search..."
        ...
        <div className="md:p-10 p-0 rounded-md flex flex-col space-y-1">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 p-8">
            {drinks.length > 0 ? (
              ...
              <p className="col-span-4 text-center">No drinks found.</p>
            )}
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 p-10">
            {meals.length > 0 ? (
              meals.map((meal) => (
                <div
                  key={meal.idMeal}
                  className="meal-item bg-white shadow-md rounded-lg overflow-hidden min-h-[300px]"
                >
                  <div className="aspect-w-1 aspect-h-1">
                    <img
                      className="aspect-w-1 aspect-h-1 object-cover"
                  ...
                  <div className="p-4">
                    <h2 className="text-lg font-light">{meal.strMeal}</h2>
                    <h2></h2>
                  </div>
                </div>
              ))
            ) : (
              <p className="col-span-4 text-center">No meals found.</p>
            )}
          </div>
        </div>
      </div>
